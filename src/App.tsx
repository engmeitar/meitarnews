...
export default function App() {
  ...
  const [results, setResults] = useState({ left: "", center: "", right: "", summary: "" });
  ...
  useEffect(() => {
    const fetchRecommended = async () => {
      try {
        const response = await fetch("https://newsapi.org/v2/top-headlines?country=us&pageSize=6&apiKey=e596c01e008d4bfeafdd8e3922247f93");
        if (!response.ok) {
        ...
        setError("Could not load live news articles. Please check your API key or network settings.");
      }
    };
    ...
    <div className="min-h-screen bg-gray-100 text-black font-serif">
      <header className="bg-white shadow sticky top-0 z-20 border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Neutral News</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 grid gap-10 bg-gray-50">
        ...
        {recommended.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
              <span role="img" aria-label="newspaper">🗞️</span> Recommended Articles
            </h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommended.map((article, index) => (
                <a
                  ...
                  className={`block border rounded-lg bg-white overflow-hidden hover:shadow-md transition-all duration-200 ${index === 0 ? 'md:col-span-2 lg:col-span-1' : ''}`}
                >
                  <img
                    src={article.image}
        ...
        {results.left && results.center && results.right ? (
          <section className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-2xl font-bold mb-4 border-b pb-2">Coverage From All Sides</h2>
          ...
          !loading && searchTerm && (
            <p className="text-sm text-gray-500">Try a different topic to see results.</p>
          )
        )}

        {results.summary && (
          <section className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-2xl font-bold mb-4 border-b pb-2">Neutral Summary</h2>
            <div className="bg-gray-50 p-4 border rounded">
              <p className="text-base leading-relaxed text-gray-800">{results.summary}</p>
            </div>
          </section>
        )}
      </main>
      ...
      <footer className="mt-12 py-6 text-center text-sm text-gray-500 border-t">
        &copy; {new Date().getFullYear()} Neutral News. All rights reserved. <br />
        Neutral News provides balanced summaries from various viewpoints and does not endorse any specific source.
      </footer>
    </div>
  );
}
